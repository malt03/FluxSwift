...
class ObservableDispatchActionTests: XCTestCase {
    ...
    func testIdentifiable() {
        ...
        let identifyRelay = PublishRelay<Int>()
        let identifyDisposable = identifyRelay.dispatchAction(id: { _ in "a" }, action: { IdentifiableStore.Set(x: $0) })
        ...
        identifyRelay.accept(2)
        XCTAssertEqual(2, a.entity.x)
        XCTAssertEqual(1, b.entity.x)
        disposable.dispose()
        identifyDisposable.dispose()
        relay.accept(3)
        identifyRelay.accept(3)
        XCTAssertEqual(2, a.entity.x)
        XCTAssertEqual(1, b.entity.x)
    }
